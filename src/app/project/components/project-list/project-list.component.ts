...
export class ProjectListComponent implements OnInit {
  ...
  get filteredProgressOptions(): ProjectProgress[] {
    if (this.role === 'RESIDENT') {
      return this.projectProgressOptions.filter(status => status !== ProjectProgress.NOT_STARTED);
    }
    return this.projectProgressOptions;
  }
    ...
    if (this.role === Role.RESIDENT) {
      ...
      this.agencyTableConfig.columns = [
        ...
        },
        {
          ...
          displayName: 'Project Assigned In',
          type: 'customText',
          customTextFn: (row) => {
            const u = row.assignedDetails;
            if (!u) return 'N/A';
            return `<strong>Name:</strong> ${u.sarpanchName},\n<strong>Gram Panchayat:</strong> ${u.gramPanchayatName}`;
          }
        },
        {
    ...
    const userRequests = projects.map(project =>
      this.userService.getUserById(project.createdBy).pipe(
        map(userResponse => {
          ...
          console.log(' 123456789 =====> ', project);
          let assignedDetails = {
            sarpanchName: project.assignedSarpanches[0]?.sarpanchName || 'N/A',
            gramPanchayatName: project.assignedSarpanches[0]?.gramPanchayatName || 'N/A'
          };
          ...
          return { ...project, createdByDetails, assignedDetails };
        })
      )
    );
  ...
  updateProjectProgressStatus(event: { projectId: string, progressStatus: string, expenditureAmount?: number, file?: File }) {
    // Prepare payload object, including expenditure if present
    const formData = new FormData();
    formData.append('projectId', event.projectId);
    formData.append('progressStatus', event.progressStatus);

    if (event.expenditureAmount !== undefined) {
      formData.append('expenditureAmount', event.expenditureAmount.toString());
    }
    if (event.file) {
      formData.append('document', event.file);
    }
    this.projectService.updateProjectProgressStatus(formData).subscribe({
      next: (response) => {
    ...
    this.projectService.updateProjectProgressStatus(formData).subscribe({
      ...
      error: (err) => {
        console.error('Status Updation Failed:', err.details);
        this.toastService.showError(err.details || 'Something went wrong');
        this.isLoading = false;
      }
    });
  ...
  onAction(action: string, element: any): void {
    ...
    switch (action) {
      ...
      case 'view profile': {
        console.log('Clicked project:', element.projectName);
        console.log('Assigned Sarpanches:', element.assignedSarpanches);

        const gramPanchayatName = element.assignedSarpanches[0]?.gramPanchayatName;
        console.log('Gram Panchayat:', gramPanchayatName);
        console.log('ON ACTION CLICK GRAM PANCHAYAT NAME ===> ' + element.assignedSarpanches[0].gramPanchayatName);
